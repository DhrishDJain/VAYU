...
const throttleJoystick = document.getElementById("joystickThrottleYaw");
const throttleHandle = document.getElementById("handleThrottleYaw");
const maxDist =
  throttleJoystick.offsetHeight / 2 - throttleHandle.offsetHeight / 2;

// Position handle at bottom center initially
throttleHandle.style.top = maxDist * 2 + "px"; // bottom position
throttleHandle.style.left = "50%";
throttleHandle.style.transform = "translate(-50%, 0)";
updateReadout();
setupJoystick(
  "joystickThrottleYaw",
  "handleThrottleYaw",
...
function sendThrottleValue(throttleValue) {
  var xhr = new XMLHttpRequest();
  xhr.open("GET", "/get?throttel=" + throttleValue, true);
  xhr.send();
}
...
document.getElementById("killSwitch").addEventListener("change", function () {
  if (this.checked) {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/get?KILL=" + true, true);
    xhr.send();
  }
});
